---
layout: page
title: Projects
---

<p class="message">
  This is a test of creating a new & separate page from the "About" screen. This is a list of some of my featured projects. Please feel free to reach out to ask questions about any of them!
</p>


# Featured Projects:
* MERITS Program CubeSat Material Testing & Analysis
* Boston Climate Action Network BERDO Compliance Analysis
* "Woke Windows" Update: Working with Boston Police Forfeiture Data
  * Worked on updating the [Woke Windows Project](https://www.wokewindows.org/)'s data.
* Carbon Footprint Tracker UI Project
* Covid-19 Policy Analysis in Azure
* L.L.Bean Kubernetes & Cloud Billing Costs Dashboard

## Some other work & projects:
* BU Mars Rover Club Preliminary Movement Systems (& other work)
* A* Algorithm for 2D Path Planning
* Dijkstra's Algorithm for 2D Path Planning
